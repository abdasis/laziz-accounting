<div>
    {{-- Care about people's approval and you will be their prisoner. --}}
    <div class="card border-top">
        <div class="card-body">
            <h3 class="card-title mb-3">
                Tambah Pembelian
            </h3>
            <form wire:submit.prevent="save">
                <div class="row mt-2">
                    <div class="col-md-3">
                        <div class="mb-2" wire:ignore>
                            <x-form-select name="supplier_id" id="supplier_id" label="Pilih Supplier" data-toggle="select2" >
                                <option value="">Pilih Supplier</option>
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}">{{ $supplier->company_name }}</option>
                                @endforeach
                            </x-form-select>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="mb-2">
                            <x-form-input label="Tanggal Transaksi" name="transaction_date" type="date"  />
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="mb-2">
                            <x-form-input label="Jatuh Tempo" name="due_date" type="date" />

                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="mb-2">
                            <x-form-input-group label="Kode Transaksi" >
                                <x-form-input-group-text>
                                    <i class="fe-clipboard"></i>
                                </x-form-input-group-text>
                                <x-form-input name="no_transaction" placeholder="Kode Transaksi" id="no_transaction" />
                            </x-form-input-group>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="mb-2">
                            <x-form-textarea label="Alamat Supplier" name="address" placeholder="Masukan Alamat" />
                        </div>
                    </div>
                </div>
                <table class="table table-sm">
                    <thead class="bg-light bg-gradient">
                        <tr>
                            <th>Produk</th>
                            <th>Diskripsi</th>
                            <th>Kuantitas</th>
                            <th>Harga Satuan</th>
                            <th>Pajak</th>
                            <th>Total Harga</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody class="border-light">
                    @foreach($inputs as $key => $value)
                        <tr>
                            <td>
                                <x-form-select name="product.{{$key}}">
                                    <option value="">Pilih Produk</option>
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}">{{ $product->name }}</option>
                                    @endforeach
                                </x-form-select>
                            </td>
                            <td>
                                <x-form-input name="description.{{$key}}"/>
                            </td>
                            <td width="10%">
                                <x-form-input name="quantity.{{$key}}" type="number"/>
                            </td>
                            <td>
                                <x-form-input name="price.{{$key}}"/>
                            </td>
                            <td width="10%">
                                <x-form-input name="tax.{{$key}}" type="number"/>
                            </td>
                            <td>
                                <x-form-input name="total_price.{{$key}}"/>
                            </td>
                            <td>
                                <button wire:click.prevent="removeForm({{$key}})" class="btn btn-action">
                                    <i class="ti ti-minus"></i>
                                </button>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
                <div class="row justify-content-between">
                    <div class="col-md-4">
                        <div class="mb-2">
                            <x-form-textarea label="Catatan" name="notes" placeholder="Masukan Catatan" />
                        </div>
                        <div class="mb-2">
                            <x-form-textarea label="Keterangan" name="message" placeholder="Masukan Keterangan" />
                        </div>
                    </div>
                    <div class="col-md-4">
                        <table class="table table-sm table-borderless">
                            <tr>
                                <td><strong>Total</strong></td>
                                <td>:</td>
                                <td class="text-end">{{rupiah($total)}}</td>
                            </tr>
                            <tr>
                                <td><strong>PPn</strong></td>
                                <td>:</td>
                                <td class="text-end">{{rupiah($ppn)}}</td>
                            </tr>
                            <tr>
                                <td><strong>Subtotal</strong></td>
                                <td>:</td>
                                <td class="text-end">{{rupiah($sub_total)}}</td>
                            </tr>
                            <tr>
                                <td wire:ignore>
                                    <x-form-select id="potongan" name="potongan" class="form-select form-select-sm" data-toggle="select2" data-placeholder="PPh">
                                        <option value="">Pilih Pajak</option>
                                        @foreach($taxes as $tax)
                                            <option value="{{$tax->name}}">{{$tax->name}}</option>
                                        @endforeach
                                    </x-form-select>
                                </td>
                                <td class="align-middle">:</td>
                                <td class="text-end">
                                    <div class="input-group">
                                        <input wire:model="potongan_nominal" type="text" class="form-control" placeholder="Potongan" aria-label="Username" aria-describedby="basic-addon1">
                                        <span class="input-group-text" id="basic-addon1">%</span>
                                    </div>
                                </td>
                            </tr>

                            <tr>
                                <th>Total Tagihan</th>
                                <th>:</th>
                                <th class="text-end">{{rupiah($total_tagihan)}}</th>
                            </tr>
                        </table>
                    </div>
                </div>
                <div class="d-flex justify-content-between mt-2">
                    <button wire:click.prevent="addForm({{$i}})" class="btn rounded border-bottom btn-light width-md">
                        <i class="fe-layers me-1"></i>
                        Tambah Produk
                    </button>
                    <button class="btn rounded border-bottom btn-primary width-md">
                        <i class="fe-save me-1"></i>
                        Perbarui Pembelian
                    </button>
                </div>
            </form>
        </div>
    </div>
    <div class="my-2 text-end">
        <small class="text-muted">
            Pembelian ini diperbarui terakhir pada tanggal {{\Carbon\Carbon::parse($purchase->updated_at)->format('d M Y')}}
        </small>
    </div>
</div>

@push('scripts')
    <script>
        Livewire.onLoad(() => {
            $('#potongan').select2().val(@this.potongan).trigger('change');
            $('#supplier_id').select2().val(@this.supplier_id).trigger('change');
        })

        $('#potongan').on('change', function (event){
            @this.set('potongan', event.target.value)
        })

        $('#supplier_id').on('change', function (event){
            @this.set('supplier_id', event.target.value)
        })

    </script>
@endpush
